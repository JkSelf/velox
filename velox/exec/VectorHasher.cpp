...
uint32_t VectorHasher::serializedStateSize() const {
  uint32_t size = sizeof(kVectorHasherStateMagic) +
      sizeof(kVectorHasherStateVersion) + sizeof(uint32_t) +
      sizeof(bool) + sizeof(rangeSize_) + sizeof(multiplier_) +
      sizeof(isRange_) + sizeof(hasRange_) + sizeof(rangeOverflow_) +
      sizeof(distinctOverflow_) + sizeof(min_) + sizeof(max_) +
  ...
  if (bloomFilter_) {
    ...
    VELOX_CHECK_NOT_NULL(
        bloomFilter, "Unexpected bloom filter implementation in VectorHasher");
    size +=
        sizeof(bool) + 3 * sizeof(uint32_t) + bloomFilter->blocksByteSize();
  }
