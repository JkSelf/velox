...
void HashTable<ignoreNullKeys>::serializeImpl(Writer& writer) const {
  ...
  writeValue(static_cast<uint32_t>(8)); // Version 8: added constructor parameters for complete reconstruction
  ...
  const auto numDependentColumns =
      allColumnTypes.size() > numKeyColumns ? allColumnTypes.size() - numKeyColumns : 0;
  ...
  if (totalRows > 0) {
    ...
    serializedRowBatches.reserve((rows_ != nullptr ? 1 : 0) + otherTables_.size());
    ...
    if (serializeNormalizedKeys) {
      VELOX_CHECK_EQ(
          normalizedKeys.size(),
          totalRows,
          "Total normalized keys mismatch");
    }
...
std::unique_ptr<HashTable<ignoreNullKeys>> HashTable<ignoreNullKeys>::deserializeImpl(
  ...
  uint32_t minTableSizeForParallelJoinBuild = readValue.template operator()<uint32_t>();
  ...
  if (numRows > 0) {
    ...
    for (size_t i = 0; i < columnTypes.size(); ++i) {
      const auto& expectedType =
          i < keyTypes.size() ? keyTypes[i] : dependentTypes[i - keyTypes.size()];
      VELOX_CHECK(
    ...
    for (uint64_t i = 0; i < numRows; ++i) {
      ...
      if (rowSize > 0) {
        ...
        table->rows_->storeSerializedRow(
            std::string_view(serializedRowBuffer.data(), rowSize),
            newRow);
      } else {
  ...
  const auto serializedHasherCount =
      readValue.template operator()<uint32_t>();
  VELOX_CHECK_EQ(
  ...
  if (!allRows.empty()) {
    if (table->hashMode_ == HashMode::kArray) {
      ...
      const auto numPages = memory::AllocationTraits::numPages(bytes);
      table->rows_->pool()->allocateContiguous(numPages, table->tableAllocation_);
