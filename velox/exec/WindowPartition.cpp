...
std::pair<vector_size_t, vector_size_t> WindowPartition::computePeerBuffers(
  ...
  if (supportRowLevelStreaming() && previousGroupLastRow_) {
    ...
    data_->extractSerializedRows(
        folly::Range(partition_.data() + (start - offsetInPartition()), 1),
        serialized);
    rowContainer->clear();
    ...
    if (!peerGroup) {
      ...
      while (nextStart <= lastPartitionRow) {
        if (peerCompare(
                partition_[start - offsetInPartition()],
                partition_[nextStart - offsetInPartition()],
                data_)) {
          break;
        }
        peerEnd++;
      ...
      for (auto j = start; j < nextStart; j++) {
        rawPeerStarts[j - offsetInPartition()] = peerStart;
        rawPeerEnds[j - offsetInPartition()] = peerEnd;
      }
    }
  }
  ...
  if (supportRowLevelStreaming()) {
    ...
    data_->extractSerializedRows(
        folly::Range(
            partition_.data() + (peerEnd - 1 - offsetInPartition()), 1),
