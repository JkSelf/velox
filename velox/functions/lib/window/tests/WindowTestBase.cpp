...
WindowTestBase::QueryInfo WindowTestBase::buildStreamingWindowQuery(
    ...
    const std::string& overClause,
    const std::string& frameClause) {
  std::cout << "the overClause is " << overClause << "\n";
  std::string functionSql =
      fmt::format("{} over ({} {})", function, overClause, frameClause);
  ...
  if (partitionByStartIdx != std::string::npos) {
    ...
    buildOrderByParameters(overClause, startIdx, length, orderByClauses);
  }
